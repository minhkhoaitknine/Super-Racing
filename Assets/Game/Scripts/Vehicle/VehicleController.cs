...
namespace SuperRacing.Vehicle
{
    ...
    public sealed class VehicleController : MonoBehaviour, IVehicleController
    {
        ...
        {
            if (vehicleBody == null || driveAssistAcceleration <= 0f || Mathf.Approximately(appliedMotorTorque, 0f))
            {
                return;
            }

            if (overForwardSpeedLimit)
            {
                return;
            }

            vehicleBody.AddForce(transform.forward * (driveInput.y * driveAssistAcceleration), ForceMode.Acceleration);
        }
